...
// StartRobot()
//
int FastResearchInterface::StartRobot(const unsigned int& ControlMode, const float& TimeOutValueInSeconds)
{
    int ResultValue = 0;

    float CommandValues[2 * NUMBER_OF_JOINTS], InitialSystemTimeValue = GetSystemTimeInSeconds(true);
    ...
    if (this->GetFRIMode() == FRI_STATE_OFF)
    {
        ...
        if (this->GetFRIMode() == FRI_STATE_OFF)
        {
            this->OutputConsole->printf("FastResearchInterface::StartRobot(): ERROR, the KRC has not opened the FRI "
                                        "yet or no UDP connection can be established.\n");
            return (ENOTCONN);
        }
    }
    ...
    {
        while (((GetSystemTimeInSeconds() - InitialSystemTimeValue) < TimeOutValueInSeconds) &&
               (!(this->IsMachineOK())) && (this->GetKRLIntValue(15) != 20))
        {
            ResultValue = this->WaitForKRCTick(((unsigned int)(this->CycleTime * 3000000.0)));
            ...
            if (ResultValue != EOK)
            {
                this->OutputConsole->printf(
                    "FastResearchInterface::StartRobot(): ERROR, the KRC unit does not respond anymore. Probably, the "
                    "FRI was closed or there is no UDP connection anymore.");
                return (ENOTCONN);
            }
        }
        ...
        {
            this->OutputConsole->printf("FastResearchInterface::StartRobot(): ERROR, command mode reached, but the "
                                        "robot could be turned on correctly. Check KRC state.");
            return (ETIME);
        }
    }
    else
    {
        this->OutputConsole->printf(
            "FastResearchInterface::StartRobot(): ERROR, could not switch to command mode after timeout.\n");
        return (ETIME);
    }
}
