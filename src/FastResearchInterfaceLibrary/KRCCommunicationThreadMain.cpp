...
{
    int SequenceCounter = 0, ResultValue = 0;
    ...
    for (;;)
    {
        ...
        if (ResultValue != 0)
        {
            ThisObject->OutputConsole->printf("FastResearchInterface::KRCCommunicationThreadMain(): ERROR during the "
                                              "reception of a UDP data package.\n");
        }
        ...
        LocalCommandData.Header.FRIReflectedSequenceCounterForUDPPackages =
            LocalReadData.Header.FRISequenceCounterForUDPPackages;
        ...
        if (ThisObject->LoggingIsActive)
        {
            ...
            ThisObject->DataLogger->AddEntry(LocalReadData, LocalCommandData);
        }
        else
        {
            pthread_mutex_unlock(&(ThisObject->MutexForLogging));
