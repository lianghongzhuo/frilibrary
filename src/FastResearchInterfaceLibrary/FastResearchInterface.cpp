...
{
    int ParameterCount = 0, FuntionResult = 0;

    struct sched_param SchedulingParamsKRCCommunicationThread, SchedulingParamsTimerThread, SchedulingParamsMainThread;

    pthread_attr_t AttributesKRCCommunicationThread, AttributesTimerThread;
    ...
    if (ParameterCount == -1)
    {
        fprintf(stderr, "FastResearchInterface::FastResearchInterface(): ERROR, an initialization file name is "
                        "required! QUITTING.\n");
        getchar();
    ...
    {
        fprintf(stderr,
                "FastResearchInterface::FastResearchInterface(): ERROR, initialization file \'%s\' is incomplete. Only "
                "%d parameters are given. QUITTING.\n",
    ...
    {
        this->OutputConsole->printf("FastResearchInterface::FastResearchInterface(): ERROR, could not start the KRC "
                                    "communication thread (Result: %d).\n",
                                    FuntionResult);
        getchar();
        exit(EXIT_FAILURE);  // terminates the process
    }
    ...
    this->DataLogger =
        new DataLogging(this->RobotName, this->LoggingPath, this->LoggingFileName, this->NumberOfLoggingFileEntries);
}
    ...
    if (this->NewDataFromKRCReceived)
    {
        ...
        if (ResultValue != EOK)
        {
            this->OutputConsole->printf(
                "FastResearchInterface::~FastResearchInterface(): ERROR, the KRC unit does not respond anymore. "
