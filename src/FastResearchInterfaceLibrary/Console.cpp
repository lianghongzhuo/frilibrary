...
// Constructor
//
Console::Console(const unsigned int& Priority, const FILE* FileHandler)
{
    ...
    pthread_create(&ConsoleThread, &ThreadAttributes, &ConsoleThreadMain, this);
    ...
    memset(this->Buffer, 0x0, 2 * CONSOLE_NUMBER_OF_BUFFER_ENTRIES * CONSOLE_BUFFER_ENTRY_SIZE * sizeof(char));
...
int Console::printf(const char* Format, ...)
{
    ...
    {
        sprintf(this->Buffer[this->BufferNumber][this->NumberOfMessages[this->BufferNumber] - 1],
                "Console::printf(): Buffer is full, skipping output!\n");
    ...
    Result = vsnprintf(this->Buffer[this->BufferNumber][NumberOfMessages[this->BufferNumber] - 1],
                       CONSOLE_BUFFER_ENTRY_SIZE - 1, Format, ListOfArguments);
    pthread_mutex_unlock(&(this->Mutex));
    pthread_cond_signal(&(this->CondVar));
