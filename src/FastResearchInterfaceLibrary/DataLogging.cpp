...
// Constructor
//
DataLogging::DataLogging(const char* RobotName, const char* LoggingPath, const char* LoggingFileName,
...
// PrepareLogging()
//
int DataLogging::PrepareLogging(const unsigned int& ControlScheme, const char* FileIdentifier)
{
    ...
    memset(TimeString, 0x0, TIME_STRING_LENGTH * sizeof(char));
    ...
    {
        sprintf(this->CompleteOutputFileString, "%s%s-%s-%s-%s", this->OutputPath, TimeString, FileIdentifier,
    ...
    else
    {
        ...
        fprintf(this->OutputFileHandler,
                "This file contains all important control values and importable to Matlab and MS Excel.\n");
        ...
        switch (this->CurrentControlScheme)
        {
            case FastResearchInterface::JOINT_POSITION_CONTROL:
                ...
                fprintf(this->OutputFileHandler,
                        "Counter	KRCTime	LocalTime	ActFJ1	ActFJ2	ActFJ3	ActFJ4	ActFJ5	ActFJ6	ActFJ7	"
                        "UDesJ1	UDesJ2	UDesJ3	UDesJ4	UDesJ5	UDesJ6	UDesJ7	ActJ1	ActJ2	ActJ3	ActJ4	"
                        "ActJ5	ActJ6	ActJ7	KDesJ1	KDesJ2	KDesJ3	KDesJ4	KDesJ5	KDesJ6	KDesJ7\n");
                break;
            case FastResearchInterface::CART_IMPEDANCE_CONTROL:
                ...
                fprintf(this->OutputFileHandler,
                        "Counter	KRCTime	LocalTime	DesKx	DesKy	DesKz	DesKa	DesKb	DesKc	DesDx	"
                        "DesDy	DesDz	DesDa	DesDb	DesDc	UDesFx	UDesFy	UDesFz	UDesFa	UDesFb	UDesFc\n");
                break;
            case FastResearchInterface::JOINT_IMPEDANCE_CONTROL:
                ...
                fprintf(this->OutputFileHandler,
                        "Counter	KRCTime	LocalTime	ActFJ1	ActFJ2	ActFJ3	ActFJ4	ActFJ5	ActFJ6	ActFJ7	"
                        "UDesPJ1	UDesPJ2	UDesPJ3	UDesPJ4	UDesPJ5	UDesPJ6	UDesPJ7	ActPJ1	ActPJ2	ActPJ3	ActPJ4	"
                        "ActPJ5	ActPJ6	ActPJ7	KDesPJ1	KDesPJ2	KDesPJ3	KDesPJ4	KDesPJ5	KDesPJ6	KDesPJ7	DesKJ1	"
                        "DesKJ2	DesKJ3	DesKJ4	DesKJ5	DesKJ6	DesKJ7	DesDJ1	DesDJ2	DesDJ3	DesDJ4	DesDJ5	"
                        "DesDJ6	DesDJ7	UDesFJ1	UDesFJ2	UDesFJ3	UDesFJ4	UDesFJ5	UDesFJ6	UDesFJ7	KOffPJ1	KOffPJ2	"
                        "KOffPJ3	KOffPJ4	KOffPJ5	KOffPJ6	KOffPJ7\n");
                break;
            case FastResearchInterface::JOINT_TORQUE_CONTROL:
                fprintf(this->OutputFileHandler, "Active control scheme: joint torque control\n\n");
                fprintf(this->OutputFileHandler,
                        "Counter	KRCTime	LocalTime	ActFJ1	ActFJ2	ActFJ3	ActFJ4	ActFJ5	ActFJ6	ActFJ7	"
                        "ActPJ1	ActPJ2	ActPJ3	ActPJ4	ActPJ5	ActPJ6	ActPJ7	UDesFJ1	UDesFJ2	UDesFJ3	UDesFJ4	"
                        "UDesFJ5	UDesFJ6	UDesFJ7\n");
                break;
            default:
                return (EINVAL);
        }
...
// AddEntry()
//
void DataLogging::AddEntry(const FRIDataReceivedFromKRC& ReceivedFRIData, const FRIDataSendToKRC& SentFRIData)
{
    unsigned int i = 0;

    this->LoggingMemory[0][this->OutputCounter % this->MaximumNumberOfEntries] =
        ReceivedFRIData.InterfaceState.FRITimeStamp;
    ...
    if (this->CurrentControlScheme == FastResearchInterface::JOINT_POSITION_CONTROL)
    {
        ...
        {
            this->LoggingMemory[2 + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                ReceivedFRIData.MeasuredData.FRIMeasuredJointTorqueVectorInNm[i];
            this->LoggingMemory[2 + 1 * NUMBER_OF_JOINTS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                DEG(SentFRIData.CommandValues.FRICommandedJointPositionVectorInRad[i]);
            this->LoggingMemory[2 + 2 * NUMBER_OF_JOINTS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                DEG(ReceivedFRIData.MeasuredData.FRIMeasuredJointPositionVectorInRad[i]);
            this->LoggingMemory[2 + 3 * NUMBER_OF_JOINTS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                DEG(ReceivedFRIData.MeasuredData.FRICommandedJointPostionVectorFromKRC[i]);
        }
    }
    ...
    if (this->CurrentControlScheme == FastResearchInterface::CART_IMPEDANCE_CONTROL)
    {
        for (i = 0; i < NUMBER_OF_CART_DOFS; i++)
        {
            ...
            this->LoggingMemory[2 + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                SentFRIData.CommandValues.FRICommandedCartesianStiffnessVector[i];
            this->LoggingMemory[2 + 1 * NUMBER_OF_CART_DOFS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                SentFRIData.CommandValues.FRICommandedNormalizedCartesianDampingVector[i];
            this->LoggingMemory[2 + 2 * NUMBER_OF_CART_DOFS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                SentFRIData.CommandValues.FRICommandedAdditionalCartesianForceTorqueVector[i];
        }
    }
    ...
    if (this->CurrentControlScheme == FastResearchInterface::JOINT_IMPEDANCE_CONTROL)
    {
        for (i = 0; i < NUMBER_OF_JOINTS; i++)
        {
            this->LoggingMemory[2 + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                ReceivedFRIData.MeasuredData.FRIMeasuredJointTorqueVectorInNm[i];
            this->LoggingMemory[2 + NUMBER_OF_JOINTS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                DEG(SentFRIData.CommandValues.FRICommandedJointPositionVectorInRad[i]);
            this->LoggingMemory[2 + 2 * NUMBER_OF_JOINTS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                DEG(ReceivedFRIData.MeasuredData.FRIMeasuredJointPositionVectorInRad[i]);
            this->LoggingMemory[2 + 3 * NUMBER_OF_JOINTS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                DEG(ReceivedFRIData.MeasuredData.FRICommandedJointPostionVectorFromKRC[i]);
            this->LoggingMemory[2 + 4 * NUMBER_OF_JOINTS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                SentFRIData.CommandValues.FRICommandedJointStiffnessVectorInNmPerRad[i];
            this->LoggingMemory[2 + 5 * NUMBER_OF_JOINTS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                SentFRIData.CommandValues.FRICommandedNormalizedJointDampingVector[i];
            this->LoggingMemory[2 + 6 * NUMBER_OF_JOINTS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                SentFRIData.CommandValues.FRICommandedAdditionalJointTorqueVectorInNm[i];
            this->LoggingMemory[2 + 7 * NUMBER_OF_JOINTS + i][this->OutputCounter % this->MaximumNumberOfEntries] =
                DEG(ReceivedFRIData.MeasuredData.FRICommandedJointPostionOffsetVectorFromKRC[i]);
        }
    }
...
int DataLogging::WriteToFile(void)
{
    ...
    unsigned int StartIndex = 0, StopIndex = 0, Counter = 0, i = 0, ElementsPerLine = 0;
