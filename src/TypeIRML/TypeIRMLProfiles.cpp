...
double TypeIRMLMath::ProfileStep1PosTri(const double& CurrentPosition, const double& CurrentVelocity,
                                        const double& MaxAcceleration, const double& TargetPosition)
{
    return ((2.0 * TypeIRMLMath::RMLSqrt(MaxAcceleration * (TargetPosition - CurrentPosition) +
                                         0.5 * pow2(CurrentVelocity)) -
             CurrentVelocity) /
            MaxAcceleration);
}
...
double TypeIRMLMath::ProfileStep1PosTrap(const double& CurrentPosition, const double& CurrentVelocity,
                                         const double& MaxVelocity, const double& MaxAcceleration,
                                         const double& TargetPosition)
{
    return ((TargetPosition - CurrentPosition) / MaxVelocity +
            (MaxVelocity - CurrentVelocity + 0.5 * pow2(CurrentVelocity) / MaxVelocity) / MaxAcceleration);
}

//************************************************************************************
// ProfileStep2V_To_Vmax()

void TypeIRMLMath::ProfileStep2V_To_Vmax(double* CurrentPosition, double* CurrentVelocity, const double& MaxVelocity,
                                         const double& MaxAcceleration, double* ElapsedTime,
                                         const bool& SignsAreInverted, TypeIRMLMath::TypeIMotionPolynomials* P)
{
    ...
    if (SignsAreInverted)
    {
        P->PositionPolynomial[P->ValidPolynomials].SetCoefficients((0.5 * MaxAcceleration), -*CurrentVelocity,
                                                                   -*CurrentPosition, *ElapsedTime);
        P->VelocityPolynomial[P->ValidPolynomials].SetCoefficients(0.0, MaxAcceleration, -*CurrentVelocity,
                                                                   *ElapsedTime);
    }
    else
    {
...
void TypeIRMLMath::ProfileStep2V_To_Zero(double* CurrentPosition, double* CurrentVelocity,
                                         const double& MaxAcceleration, double* ElapsedTime,
                                         const bool& SignsAreInverted, TypeIRMLMath::TypeIMotionPolynomials* P)
{
    double TimeForThisSegment = 0.0;
...
void TypeIRMLMath::ProfileStep2PosTrap(double* CurrentPosition, double* CurrentVelocity, const double& MaxAcceleration,
                                       const double& TargetPosition, const double& SynchronizationTime,
                                       double* ElapsedTime, const bool& SignsAreInverted,
                                       TypeIRMLMath::TypeIMotionPolynomials* P)
{
    double HoldVelocity = 0.0, Time2 = 0.0, Time3 = 0.0, Position2 = 0.0, Position3 = 0.0;

    // Calculate all trajectory parameters

    HoldVelocity = 0.5 * (MaxAcceleration * (SynchronizationTime - *ElapsedTime) + *CurrentVelocity) -
                   0.5 * TypeIRMLMath::RMLSqrt(
                             4.0 * MaxAcceleration * (*CurrentPosition - TargetPosition) +
    ...
    *ElapsedTime = Time2;
    ...
    if (SignsAreInverted)
    {
        P->PositionPolynomial[P->ValidPolynomials].SetCoefficients(0.0, -*CurrentVelocity, -*CurrentPosition,
                                                                   *ElapsedTime);
...
void TypeIRMLMath::ProfileStep2NegHldNegLin(double* CurrentPosition, double* CurrentVelocity,
                                            const double& MaxAcceleration, const double& TargetPosition,
                                            const double& SynchronizationTime, double* ElapsedTime,
                                            const bool& SignsAreInverted, TypeIRMLMath::TypeIMotionPolynomials* P)
{
    double HoldVelocity = 0.0, Time2 = 0.0, Time3 = 0.0, Position2 = 0.0, Position3 = 0.0, Denominator = 0.0;

    // Calculate all trajectory parameters

    Denominator = (2.0 * MaxAcceleration * (SynchronizationTime - *ElapsedTime) - 2.0 * *CurrentVelocity);
    ...
    {
        HoldVelocity =
            (2.0 * MaxAcceleration * (TargetPosition - *CurrentPosition) - pow2(*CurrentVelocity)) / Denominator;
