...
int TypeIRML::GetNextMotionState_Position(const double* CurrentPosition, const double* CurrentVelocity,
                                          const double* MaxVelocity, const double* MaxAcceleration,
                                          const double* TargetPosition, const bool* SelectionVector,
                                          double* NewPosition, double* NewVelocity)
{
    int i = 0, ReturnValue = TypeIRML::RML_ERROR;
    ...
    TypeIRML::SynchronizeTrajectory(*(this->CurrentInputParameters), MinimumSynchronizationTime, this->Polynomials);

    // *******************************************************************************
    // * Step 3: Calculate output values
    // *******************************************************************************

    ReturnValue = TypeIRML::CalculateOutputValues(this->Polynomials, *(this->CurrentInputParameters->SelectionVector),
                                                  OutputParameters);
...
int TypeIRML::GetNextMotionState_Position(const TypeIRMLInputParameters& IP, TypeIRMLOutputParameters* OP)
{
    return (GetNextMotionState_Position(
        IP.CurrentPosition->VecData, IP.CurrentVelocity->VecData, IP.MaxVelocity->VecData, IP.MaxAcceleration->VecData,
        IP.TargetPosition->VecData, IP.SelectionVector->VecData, OP->NewPosition->VecData, OP->NewVelocity->VecData));
}
...
double TypeIRML::CalculateMinimumSynchronizationTime(const TypeIRMLInputParameters& IP) const
{
    ...
    double CurrentPosition = 0.0, CurrentVelocity = 0.0, MaxVelocity = 0.0, MaxAcceleration = 0.0, TargetPosition = 0.0,
           ExecutionTimeForCurrentDOF = 0.0, MinimumExecutionTime = 0.0;
    ...
    for (i = 0; i < this->NumberOfDOFs; i++)
    {
        if ((*IP.SelectionVector)[i])
        {
            ...
            if (!TypeIRMLMath::Decision_1002(CurrentVelocity, MaxVelocity))
            {
                // v --> vmax
            ...
            if (!TypeIRMLMath::Decision_1003(CurrentPosition, CurrentVelocity, MaxAcceleration, TargetPosition))
            {
                // v --> 0
            ...
            if (TypeIRMLMath::Decision_1004(CurrentPosition, CurrentVelocity, MaxVelocity, MaxAcceleration,
                                            TargetPosition))
            {
                ExecutionTimeForCurrentDOF += TypeIRMLMath::ProfileStep1PosTrap(
                    CurrentPosition, CurrentVelocity, MaxVelocity, MaxAcceleration, TargetPosition);
            }
            else
            {
                ExecutionTimeForCurrentDOF +=
                    TypeIRMLMath::ProfileStep1PosTri(CurrentPosition, CurrentVelocity, MaxAcceleration, TargetPosition);
            }
        }
...
void TypeIRML::SynchronizeTrajectory(const TypeIRMLInputParameters& IP, const double& SynchronizationTime,
                                     TypeIMotionPolynomials* PolynomialArray)
{
    ...
    for (i = 0; i < this->NumberOfDOFs; i++)
    {
        if ((*IP.SelectionVector)[i])
        {
            ...
            if (!TypeIRMLMath::Decision_2002(CurrentVelocity, MaxVelocity))
            {
                TypeIRMLMath::ProfileStep2V_To_Vmax(&CurrentPosition, &CurrentVelocity, MaxVelocity, MaxAcceleration,
                                                    &ElapsedTime, SignsWereSwitched, &(PolynomialArray[i]));
            }

            if (TypeIRMLMath::Decision_2003(CurrentPosition, CurrentVelocity, MaxAcceleration, TargetPosition))
            {
                if (TypeIRMLMath::Decision_2004(CurrentPosition, CurrentVelocity, MaxAcceleration, TargetPosition,
                                                SynchronizationTime))
                {
                    TypeIRMLMath::ProfileStep2PosTrap(&CurrentPosition, &CurrentVelocity, MaxAcceleration,
                                                      TargetPosition, SynchronizationTime, &ElapsedTime,
                                                      SignsWereSwitched, &(PolynomialArray[i]));
                }
                else
                {
                    TypeIRMLMath::ProfileStep2NegHldNegLin(&CurrentPosition, &CurrentVelocity, MaxAcceleration,
                                                           TargetPosition, SynchronizationTime, &ElapsedTime,
                                                           SignsWereSwitched, &(PolynomialArray[i]));
                }
            }
            ...
            {
                TypeIRMLMath::ProfileStep2V_To_Zero(&CurrentPosition, &CurrentVelocity, MaxAcceleration, &ElapsedTime,
                                                    SignsWereSwitched, &(PolynomialArray[i]));
                ...
                TypeIRMLMath::ProfileStep2PosTrap(&CurrentPosition, &CurrentVelocity, MaxAcceleration, TargetPosition,
                                                  SynchronizationTime, &ElapsedTime, SignsWereSwitched,
                                                  &(PolynomialArray[i]));
            }
        }
    }
...
int TypeIRML::CalculateOutputValues(const TypeIMotionPolynomials* PolynomialArray,
                                    const TypeIRMLBoolVector& SelectionVector, TypeIRMLOutputParameters* OP)
{
    int i = 0, SegmentCounter = 0, ReturnValue = TypeIRML::RML_FINAL_STATE_REACHED;
    ...
    for (i = 0; i < this->NumberOfDOFs; i++)
    {
        ...
        if (SelectionVector[i])
        {
            ...
            (*OP->NewPosition)[i] =
                PolynomialArray[i].PositionPolynomial[SegmentCounter].CalculateValue(this->CycleTime);
            (*OP->NewVelocity)[i] =
                PolynomialArray[i].VelocityPolynomial[SegmentCounter].CalculateValue(this->CycleTime);
...
int TypeIRML::GetNextMotionState_Velocity(const double* CurrentPosition, const double* CurrentVelocity,
                                          const double* MaxAcceleration, const double* TargetVelocity,
                                          const bool* SelectionVector, double* NewPosition, double* NewVelocity)
{
    int i = 0, ReturnValue = TypeIRML::RML_FINAL_STATE_REACHED;
        ...
        if (SelectionVector[i])
        {
            ...
            else
            {
                ...
                NewPosition[i] = CurrentPosition[i] +
                                 0.5 * MinimumExecutionTime * (CurrentVelocity[i] - TargetVelocity[i]) +
...
int TypeIRML::GetNextMotionState_Velocity(const TypeIRMLInputParameters& IP, TypeIRMLOutputParameters* OP)
{
    return (GetNextMotionState_Velocity(
        IP.CurrentPosition->VecData, IP.CurrentVelocity->VecData, IP.MaxAcceleration->VecData,
        IP.TargetVelocity->VecData, IP.SelectionVector->VecData, OP->NewPosition->VecData, OP->NewVelocity->VecData));
}
