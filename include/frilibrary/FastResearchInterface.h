...
//! <li>Providing shared KRL variables <li>Providing communication status and quality data (e.g., jitter, latency, etc.;
//! measured by the KRC host) <li>Setting robot control data (e.g., commanded joint positions, commanded joint torques,
//! etc.) <li>Setting shared KRL variables <li>Performing the start-up procedure of the robot <li>Performing the
//! shutdown procedure of the robot <li>Enabling low-level real-time data logging methods <li>Providing two different
//! timer signals (KRC timer and local timer) <li>A real-time capable method for console output
//! </ul>
//! </ul>
...
public:
    ...
    //! <li> Once, the KRC unit runs in <em>Monitor Mode</em>, the method sets the KRL variable
    //! <tt>$FRI_FRM_INT[16]</tt> to a value of \c 10, which lets the KRL program \c FRIControl call the function
    //! <tt>friStart()</tt> in order to switch from <em>Monitor Mode</em> to <em>Command Mode</em>. <li> In the
    //! transition phase, the following command variables are sent from the remote host to the KRC unit (see also:
    //! FastResearchInterface::SetControlScheme()):\n\n <ul> <li> Joint position control <ul> <li> Desired joint
    //! position vector = Current joint position vector
    //! </ul>\n
    ...
    //! already called by a previous call of FastResearchInterface::StartRobot(). If the robot is not ready for
    //! operation (i.e., the result of FastResearchInterface::IsMachineOK() is \c false), call
    //! FastResearchInterface::StopRobot() first. It might be required to check whether the KRC unit is running
    //! correctly, and whether the KRL program KRL program \c FRIControl (cf.\ref sec_KRLFile1) is being executed. <li>
    //! \c ETIME if the start-up procedure could not be completed within the specified time interval of \c
    //! TimeOutValueInSeconds <li> \c EOK if no error occurred.
    //! </ul>
    //!
